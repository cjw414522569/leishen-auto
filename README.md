...
| `RUN_INTERVAL` | — | **仅本地**：定时运行的间隔（如 `24h`）；留空则跑一次就退出 |
...
默认 `python main.py` **只运行一次就退出**。想让它常驻，在 `.env` 里设 `RUN_INTERVAL`：

```bash
RUN_INTERVAL=24h      # 每 24 小时跑一次
```

支持 `30m` / `24h` / `90s` / `2d`，不带单位按秒算。设了之后进程会一直跑，按 `Ctrl+C` 退出：

```
⏰定时模式：每 1 天 运行一次，按 Ctrl+C 退出
[2026-09-11 21:53:28] ⌛️开始运行
[2026-09-11 21:53:28] 🔑复用本地缓存的令牌（有效期至 2026-09-18 12:50:10）
[2026-09-11 21:53:28] 👌已经暂停: 400803 - 账号已经停止加速，请不要重复操作
[2026-09-11 21:53:28] 😴下次运行：2026-09-12 21:53:28
```

几点说明：

- **常驻期间每行日志都带时间戳**，翻日志时好定位
- **启动后先跑一轮**，再按间隔休眠（不是先等一个间隔）
- 每轮的令牌都走缓存，不会反复登录
- 单轮里出现意外异常**不会让进程退出**，只记一行然后等下一轮
- 这是**间隔**语义，不是"每天凌晨 1 点"那种定时。要对齐到某个时刻，
  用云函数的定时触发器（它支持 cron）
